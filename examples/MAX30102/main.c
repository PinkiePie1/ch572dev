...
measure:
	...
	memset(dat,0x00,4);//clear fifo
	MAX30102_WriteReg( 0x04, dat, 4);

	dat[0] = 0xC0;
	MAX30102_WriteReg( 0x02, dat, 1);//set interrupt

	//LED drive pulse amplitude
	dat[0] = 0x4F;
	dat[1] = 0x4F;
	MAX30102_WriteReg( 0x0C, dat, 2);

	dat[0] = 0x02;//heartrate mode,start data reading.
	MAX30102_WriteReg( 0x09, dat, 1);

	uint16_t samples_to_collect = 152;
	uint16_t samples_to_ignore = 3;//ignore a few samples
	uint8_t l = 0;
	...
	memset(imageCache,0x00,2888);//clear page

	uint32_t average = Meas_Data[0];
	uint16_t prev_index = 256;
	uint16_t HR = 18;
	for(uint16_t i = 0; i < 149; i++ )
	{
		average = average - (average>>4) + (Meas_Data[i]>>4);
		if (Meas_Data[i] > average && Meas_Data[i+1] < average)
		{
			drawLine( i, 130, i, 152, BLACK);
			if(prev_index==256){prev_index=i;}
			else
			{
				HR = (HR >> 1) + ( (i - prev_index) >> 1 ); 
				prev_index = i;
			}
		}
	}
	...
	EPD_Printf( 0, 0, font16, BLACK, "HR monitor:%d BPM",1500/HR );

...
__attribute__((__aligned__(4))) uint8_t imageCache[2024]={0};
...
measure:
	...
	if(imageCache != NULL)
	{
		...
		memset(imageCache,0x00,2024);
		/*
		switch (img_index)
		{
		case 0:
			memcpy(imageCache,gImage_mi1,2024);
			textcolor = BLACK;
			break;
		case 1:
			memcpy(imageCache,gImage_mi2,2024);
			textcolor = BLACK;
			break;
		case 2:
			memcpy(imageCache,gImage_cat,2024);
			textcolor = BLACK;
			break;
		default:
			break;

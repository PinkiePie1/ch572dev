...
{	
    ...
    {	
		index = (xStart>>3)*296 + i;
		tmp = image[index];
		image[index] = color ? ( tmp | mask2 ) : ( tmp & mask2 );

		if( i==yStart || i==yEnd )
		{
			tmp = image[(xStart>>3)*296+i];
			image[(xStart>>3)*296+i] = color ? 
			                            (tmp | (0xFF>>(xStart&7))) :
			                            (tmp & ~(0xFF>>(xStart&7)));
			                            
			tmp = image[(xEnd>>3)*296+i];
			image[(xEnd>>3)*296+i] = color ?
			                          (tmp | ~(0xFF>>(xEnd&7))) :
			                          (tmp & (0xFF>>(xEnd&7))); 
...
{	
	...
	{	
    	uint16_t index = ((xStart>>3)*296) + yStart;
		if(color){
	...
	{	
		...
		if(color)
		{
			for(uint8_t i = 0; i< 16; i++)
			{
				...
				image[index+296+i] &= ~(0xFF<<(8-offset));
				image[index+296+i] |= (temp<<(8-offset));
			}
		}
		else
		{
